...
// API (mount BEFORE static & fallback)
try {
  const api = require('./backend/api');
  app.use('/api', api);
} catch (e) {
  console.warn('API router not found or failed to load:', e?.message);
}

// Static
app.use(express.static(PUBLIC_DIR, { index: 'index.html', extensions: ['html','htm'] }));

// Health (platform & uptime)
...
const HOST = '0.0.0.0'; // Render binds on 0.0.0.0
